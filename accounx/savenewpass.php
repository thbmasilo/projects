...
if(isset($_GET['u']) && isset($_GET['p'])){
	...
	$enddate=encrypt_decrypt('decrypt',mysqli_real_escape_string($db_conx,$_GET['d']));
	$today = date("d,m,Y");

	if($enddate < $today){ //checking if the link hasn't expired.
		echo"";
		output('Forgot Password', 'Sorry the link has expired. Please restart the process again', 'forgotpassword.html');
		exit();
	}
	...
	$u = encrypt_decrypt('decrypt',mysqli_real_escape_string($db_conx,stripslashes($_GET['u'])));
	$temppasshash= mysqli_real_escape_string($db_conx,stripslashes($_GET['p']));
	$id=  encrypt_decrypt('decrypt',mysqli_real_escape_string($db_conx,stripslashes($_GET['id'])));
	...
	if($numrows == 0){
		output('Error', 'You not found in the system.', '#');
    	exit();
	} else {
		$row = mysqli_fetch_row($query);
		$id = $row[0];
		$sql = "UPDATE users SET Password='$temppasshash' WHERE UserID='$id' LIMIT 1";
	    $query = mysqli_query($db_conx, $sql);

		output('Login', 'Process Completed', 'http://maganaprepaidmeter.co.za/accounx/index.php');//displaying the result.
        exit();
    }
}
else
{
	output('Login', 'You Should not be on this page.', 'http://maganaprepaidmeter.co.za/accounx/index.php');
}

function output($buttonText, $StatusText, $urlText){
	echo '
			<!DOCTYPE html>
			<html>
...
<link rel="icon" href="pictures/title_pic.png" type="image/png">
			...
			<body>
			  ...
			  <section id="box" >
				...
				<button type="submit" value="Login">'.$buttonText.'</button>
				</form>
			  </section>
<div id="explosion2">

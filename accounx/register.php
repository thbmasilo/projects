...
if(isset($_GET['sub_id']))	{
	...
	$sub_id = encrypt_decrypt('decrypt',mysqli_real_escape_string($db_conx, $_GET['sub_id']));//protects from sql injection attacks.
	$sql = "SELECT `OrganizationID` FROM `subscriptions` WHERE `SubscriptionID` = '$sub_id' AND `Status`= true";
	$query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
	if($numrows == 0){
		output('The subscription Key you provided does not exist or it has expired. Please contact the accounx office.');
    	exit();
	} else {
		$row = mysqli_fetch_row($query);
		$org_id = $row[0];
		$encrypted_id = encrypt_decrypt('encrypt',$sub_id);
		display($sub_id = $encrypted_id);
...
function output($StatusText){
	echo '
			<!DOCTYPE html>
			<html>
			<!--http://www.formget.com/form-shake-effect-on-invalid-entry-using-jquery/-->
			<head>

			  <meta charset="UTF-8">

			  <title>User Registration</title>
<link rel="icon" href="pictures/title_pic.png" type="image/png">
		...
		<link href="css/shakestyle.css" rel="stylesheet">
		...
		<script>
			$(document).ready(function() {
				...
				{
					//Hide the legs
					document.getElementById("red-legs").style.display = "none";
					document.getElementById("orange-legs").style.display = "none";

				}
				else
				{
					document.getElementById("holder").style.display = "none";
					document.body.style.backgroundImage = "url(\'pictures/safariback.png\')";
				}
			});
			</script>
			</head>
			...
			<body>
			  <section id="box" >
			   <a href="http://maganaprepaidmeter.co.za/accounx/index.php">
                           <circle>
		            <img src="pictures/spinning_pic.jpeg" alt="ACCOUNX.COM" height="48" width="48" >
	                   </circle>
                            </a>
                           <img src="http://maganaprepaidmeter.co.za/accounx/pictures/login_pic.PNG" alt="ACCOUNX.COM"></br></br>
				  <h1 id="statusMessage" >'.$StatusText.'</h1>
			  </section>
<div id="explosion2">
			<div id="explosion-circle2"></div>
	</div>
	<div id="explosion3">
			<div id="explosion-circle3"></div>
	</div>
	<div id="explosion4">
			<div id="explosion-circle4"></div>
	</div>
<div id="holder">
	            <div id="viewport">
	                ...
	                <div class="box dude" style="
	                     "> <div class="face" style="
	                        "></div><p style="padding: 15 0 15 0">Liabilities</p></div>
...
<link rel="icon" href="pictures/title_pic.png" type="image/png">
		...
		<script>
				...
				if(navigator.userAgent.indexOf("Chrome") != -1 )
					...
					$("#login_button").click(function()
					{
						...
						var dataString = "name="+name+"&surname="+surname+"&cell_num="+cell_num+"&email="+email+"&password="+password+"&sub_id="+sub_id;
						if(($.trim(name).length>0)&&($.trim(surname).length>0)&&($.trim(cell_num).length>0)&&($.trim(email).length>0)&&($.trim(password).length>0)&&($.trim(password2).length>0))
						{
							if(password === password2)
							{
								if($.trim(password).length>8)
								{
									if(parseInt(cell_num))
									{
										$.ajax({
											...
											data: dataString,
											cache: false,
											beforeSend: function(){ $("#login_button").val("Connecting..."); document.getElementById("login_button").disabled = true;},
											success: function(data){
												if(data==="true")
												{
													document.getElementById("name").value="";
													document.getElementById("surname").value="";
													document.getElementById("cell_num").value="";
													...
													swal({ title:"Successfuly added you as a user:) ",   text: "Lets go to the login page...",   timer: 3000,   showConfirmButton: false });
													setTimeout(function () {
														window.location="http://maganaprepaidmeter.co.za/accounx/index.php";
													}, 3000);
												}
												else
												{
									...
									{
										sweetAlert("Oops...", "You have to enter your cellphone number correctly", "error");
										$("#box").shake();
										return false;
									}
								}
								...
								{
									sweetAlert("Oops...", "The password has to be longer than 8 characters", "error");
									$("#box").shake();
									return false;
								}
							}
							...
							{
								sweetAlert("Oops...", "The passwords you entered do not match", "error");
								$("#box").shake();
								return false;
	...
	<body>
	  <section id="box" >
		...
		<input placeholder="Cell Number" type="text" name="cell_num" id="cell_num">
		...
		<button type="submit" id="login_button" value="Register" >Register</button>
